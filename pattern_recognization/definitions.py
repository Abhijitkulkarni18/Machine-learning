# -*- coding: utf-8 -*-
"""
Created on Mon Dec 17 17:26:52 2018
...
#            print(e)
#            avgOutcome = 0
#        futureOutcome = percentChange(currentPoint, outcomeRange)
...
#        performanceAr.append(futureOutcome)
        ...
        y+=1
    with open('threshold_storage.pickle','wb') as f:
        _pickle.dump(threshold,f)
    with open('pattern_storage.pickle','wb') as f:
...
def patternRecognition(patForRec:list):

    plotPatAr = []
    patFound = 0
    with open('pattern_storage.pickle','rb') as f:
    ...
    #        fig = plt.figure(figsize=(10,6))
            ...
            for eachPatt in plotPatAr:
                ...
                with open('threshold_storage.pickle','rb') as f:
                    threshold= _pickle.load(f)
                if threshold[futurePoints] >=190:
                    print("-----------------Alert--------------------------")
...
def predict_value(toWhat,dataLength,bid,ask):
...
#        threshold = []
        ...
        patternStorage(avgLine)
        currentPattern(avgLine,patForRec=patForRec)
        patternRecognition(patForRec=patForRec)
